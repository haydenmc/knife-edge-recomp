***Disclaimer: This is what those in the decomp/recomp community would call an
"AI Slop" recomp. It is almost entirely LLM-generated by a mix of Claude Fable,
Opus 5, and Sonnet.***

As a career Software Engineer, I think it's remarkable that an LLM can leverage
the amazing tools built by the retro reverse-engineering community to derive a
static recompilation of an N64 game.

However, this project has not been built, reviewed, or endorsed by anyone with
relevant expertise in the area.

I can make no guarantees of the quality, accuracy, stability, or security of
the code in this repository, since I did not write it, and I lack the knowledge
to properly review it.

This was merely an exercise in curiosity to see if I could get an LLM to pop out
a PC port of an obscure N64 title I remember from my childhood.

I hope to some day develop the skills necessary to create a proper decomp
project.

For examples of decomps/recomps that *were* built by communities of experts,
check out:
...
- [Starfox64Recomp](https://github.com/sonicdcer/sf64)
