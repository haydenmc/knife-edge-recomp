...
| `hud_relocation` | Re-anchors the in-mission HUD to the true window edges instead of the centered 4:3 column that `widescreen` creates and the 320x200-era band edges that `full_height` opens up. Health cluster anchors LEFT, S-BOMB gauge RIGHT, each keeping its own original margin (20 px / 25 px); the cutscene radio box and the aiming reticle anchor CENTER, i.e. exactly where they already are; the full-screen flash stretches edge to edge. The vertical half (+20 px on the bottom clusters, -20 px on the radio box) applies **only** when `full_height` is also on -- with the letterbox still present the elements are already flush against it. Mechanism: every HUD element is one `G_TEXRECT` sub-display-list at a fixed RDRAM address, called by a single `G_DL` push from the frame's main list (`analysis/docs/hud-relocation.md` phase 1), so `RT64Context::send_dl` walks the frame, repoints each such push at a 4-command stub in scratch RDRAM (`gEXSetRectAlign(anchor)` / call the real element / `gEXSetRectAlign(reset)` / `G_ENDDL`), and injects an extended-GBI prologue that also re-expresses the frame's scissor against the window edges. Nothing in the game's own coordinates, aim math or globals changes -- it is purely where RT64 places the rectangles. Full design + measurements: `analysis/docs/hud-relocation.md` phase 2. | `widescreen` and `full_height` each expose screen area the HUD does not follow it into: the HUD stays inside the centered 4:3 column with wide black-ish margins beside it, and floats 20 px above the true bottom edge. | (c) RT64 renderer options (`hr_option`, and the extended-GBI features it gates) **and** (d) runtime shim (the per-frame redirect pass and the scratch-RDRAM stubs) | off |
...
by construction. **Owner hands-on verification pending**, like its siblings
were at this stage. Two things phase 2 measured that are worth knowing before
touching this code:
