...
| `hud_relocation` | Re-anchors the in-mission HUD to the true window edges instead of the centered 4:3 column that `widescreen` creates and the 320x200-era band edges that `full_height` opens up. Health cluster anchors LEFT, S-BOMB gauge RIGHT, each keeping its own original margin (20 px / 25 px) and each taking its dynamic fill with it; the aiming reticle anchors CENTER, i.e. exactly where it already is; the full-screen damage flash stretches edge to edge. The in-mission cutscene radio box is **deliberately left exactly where the game draws it** (owner decision 2026-07-31 -- phase 2 moved it, and the cutscene's own graphical effects, which are authored against the 4:3 column, read as misaligned beside it). The vertical half (+20 px on the bottom clusters) applies **only** when `full_height` is also on -- with the letterbox still present the elements are already flush against it. Mechanism: every HUD element is one `G_TEXRECT` sub-display-list called by a single `G_DL` push from the frame's main list (`analysis/docs/hud-relocation.md` phase 1), so `RT64Context::send_dl` walks the frame, repoints each such push at a 4-command stub in scratch RDRAM (`gEXSetRectAlign(anchor)` / call the real element / `gEXSetRectAlign(reset)` / `G_ENDDL`), and injects an extended-GBI prologue that also re-expresses the frame's scissor against the window edges. Static elements are matched by sub-DL address; the two **dynamic fills** (health arc, S-BOMB charge) are not, because the game draws them from a pool of 0x240-byte display-list buffers whose slot changes with the value shown -- thirteen and three addresses observed, and not all of them -- so those are matched **geometrically**, by their texrects falling inside the cluster's own footprint (phase 3). Nothing in the game's own coordinates, aim math or globals changes -- it is purely where RT64 places the rectangles. Multiplayer-aware since phase 4: all four players' reticles carry the bias-undoing stub, and **BATTLE has its own anchor set** -- two teams, each cluster (dial + digits + fill + gauge) anchored to its own side's window edge as a rigid unit, which is why the S-BOMB gauge is one of the two addresses with layout-dependent anchors (`Right` in single player / TEAM, `Left` in BATTLE); the other is the damage flash, which in BATTLE is two half-frame rects, one per team, each stretched to its own half of the window instead of the single full-frame rect single player and TEAM draw. A frame is identified as BATTLE by the presence of team B's mirror sub-DLs in it. Full design + measurements: `analysis/docs/hud-relocation.md` phases 2-4 and `analysis/docs/multiplayer.md` phase 3. | `widescreen` and `full_height` each expose screen area the HUD does not follow it into: the HUD stays inside the centered 4:3 column with wide black-ish margins beside it, and floats 20 px above the true bottom edge. | (c) RT64 renderer options (`hr_option`, and the extended-GBI features it gates) **and** (d) runtime shim (the per-frame redirect pass and the scratch-RDRAM stubs) | off |
| `extended_aim` | Lets the aiming reticle travel to the whole visible viewport instead of the original +-128 / +-84 N64-pixel rails, which are 80% of the half-width of the 4:3 view and 70% of the half-height of the 320x200 mission view. The rails scale by exactly how much the view opened -- `128 * aspect/(4:3)` (**+-170** at 16:9, from the live window aspect, never below the vanilla 128) and `84 * 240/200` = **+-100** -- so the game's own edge margins and its `1 unit = 1 N64 pixel` mapping both survive, and every measured constant in `analysis/docs/mouse-aim.md` stays literally valid. **Not a presentation change:** this is the only flag here that changes what the player can *do*, since more of the screen becomes aimable. Three parts, of which only two needed a patch: the clamp (two hooks bracket `func_8016E520_1501A0`'s four-`slti` clamp block -- the first stashes the unclamped pair, the second re-clamps it to our rails); the **aim ray, which needs nothing** (the game maps reticle units to a direction with `atan2(pixel, 120/tan(FOV/2))`, and that focal length is the rendering frustum's own, so crosshair and shot stay registered at any value); and the draw, where the game's *sprite library* -- not the RDP -- re-asserts a software clip box of (0,0)-(319,239) on every blit and clipped or dropped the reticle past it, so an entry hook on the blit shifts the reticle's x by a bias B = rail-128 (keeping the emitted 12-bit *unsigned* texrect coordinates non-negative) and a callee hook on `spScissor` widens the box for that one call, with `hud_relocation`'s reticle stub subtracting the same B at draw time. Horizontal half gated on `widescreen` **and** `hud_relocation` (that stub, plus its prologue's scissor re-expression, IS the draw side); vertical half gated on `full_height` and needs no draw change at all. The blit hook biases **all four** players' reticles (it matches the blit's sprite against every entry of the game's table at `0x8019D864`), so all four need the un-biasing stub -- listing only P1's, which is what shipped before multiplayer could be entered, left P2-P4 drawn 129 host px right and sliced at the rail. Full RE + patch plan + measurements: `analysis/docs/hud-relocation.md`, "Reticle range extension" and phase 4; `analysis/docs/multiplayer.md` phase 3. | `widescreen` and `full_height` make the frame bigger but leave the reticle stopping where a 4:3 / 320x200 frame ended, so a visible band of newly-rendered scene down each side and along the top and bottom cannot be aimed at. | (a) N64Recomp hooks via `gen_syms.py` (four; `src/main/extended_aim.cpp`) **and** (d) runtime shim (the resolved rails, the mouse controller's target clamp, and the reticle's relocation stub offset) | off |
...
Notes on `hud_relocation`: in the `enhanced` profile's curated set, on the
same reasoning as `full_height` -- it exists to finish what `widescreen` and
`full_height` start, and on its own (either of those off) it is a near-no-op
by construction. Owner hands-on 2026-07-31 produced one bug (the two dynamic
fills stayed behind -- fixed in phase 3 by matching them geometrically rather
than by address) and one design call (**the cutscene radio box is
deliberately left in place**, see the table row); a re-check of the fixed
build is pending. Three things worth knowing before touching this code:

* **The table and the region list are keyed by layout, not just by address.**
  Single player and TEAM share one HUD exactly; BATTLE draws two teams' worth
  at partly the *same* addresses but different coordinates, so one element
  (the S-BOMB gauge) carries two anchors and each region box states which
  layout it describes. Reusing single player's S-BOMB box in BATTLE was not
  merely useless there -- it is where team B's health dial lives, and team B's
  fill was being relocated as if it were the charge fill.
* **Do not add an address to the fingerprint table for anything whose picture
  depends on a game value.** Phase 2 did exactly that for the health fill arc
  and the S-BOMB charge fill, using the two and one addresses its captures
  happened to see, and the result was an element that relocated correctly at
  some health values and stayed behind at others. Those elements come out of
  a shared display-list buffer pool and have no fixed address; they are
  matched by region instead (`hud_regions[]`). The table is for static
  elements only.

* It is the first enhancement that **writes into game RAM** -- the `G_DL`
  push words in the frame's main display-list buffer. That buffer is rebuilt
  wholesale every frame at a per-frame-varying offset (52 distinct offsets
  over 150 dumped frames) and never read back by game code, so a rewritten
  word is dead the moment the frame is submitted; the element sub-display-
  lists themselves are never written. The alternative -- copying the ~200
  command main buffer into scratch and editing the copy -- was considered and
  rejected as strictly more work for the same guarantee (phase 1 open
  question O6).
* It sets `hr_option = Full` (RT64 `extAspectRatio = Expand`) when
  `widescreen` is also on. That knob is consulted *only* for rects carrying
  an extended-GBI origin, which no game rect ever does, so it changes nothing
  outside the stubs -- with `Original` the same stubs are an exact identity
  transform instead. Measured both ways; see `hud-relocation.md` phase 2.

Notes on `extended_aim`: in the `enhanced` profile's curated set pending the
owner's hands-on check, the same way `hud_relocation` was. Two things to know
before touching it:

* **The off-path is the fidelity guarantee.** All four hooks are compiled into
  the recompiled code in *every* profile -- there is no "not built in" here --
  so vanilla-faithfulness lives entirely in their early-outs, each a single
  relaxed atomic load against the resolved rails. Measured empirically:
  `--profile vanilla` rails at exactly +-128 / +-84, and prints no `[hud]`
  line at all.
* **The rail is one number and everything else is derived from it.** The draw
  bias is `rail - 128`, computed at each use rather than stored, so the
  game-side shift and the stub offset that undoes it cannot drift apart (risk 2
  in the RE doc's register). The rail itself comes from the *live* window
  aspect, republished by `update_gfx()` on resize, because a rail from a stale
  aspect would let the player aim at world the current window does not render.
  The second place the rail lives is the positional mouse controller's target
  clamp (`src/main/main.cpp`), which reads the same resolved value -- with the
  old constant left there, mouse aim would have stopped at 128 while keyboard
  and pad went to 170, and it would have read as a mouse bug.
